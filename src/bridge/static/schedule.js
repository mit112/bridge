...
function paintEditInput(panel) {
  ...
  if (Number.isFinite(epoch)) input.value = epochToDatetimeLocalValue(epoch);
}
...
document.addEventListener("click", async (event) => {
  ...
  if (scheduleSubmit) {
    ...
    const scheduledFor = when ? localInputToEpoch(when.value) : null;
    if (!scheduledFor) {
  ...
  if (editSave) {
    ...
    const epoch = when ? localInputToEpoch(when.value) : null;
    if (!epoch) {
      announce(key, "⚠ Pick a date and time first");
      return;
    }
