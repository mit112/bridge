...
A monotonic revision guarded by a lock. Producers `bump()` it; the SSE loop
waits on it instead of sleeping. The lost-wakeup guarantee lives in the wait:
it compares `revision > since` UNDER the lock before parking, so a bump landing
between a waiter reading `since` and calling wait is never slept through.

`wait_async()` is the only wait: every caller is on an event loop, so it parks
on an `asyncio.Event` bound to the loop that registered it and an idle SSE
...
and wakes the waiters through `loop.call_soon_threadsafe`.
"""
...
class ChangeNotifier:
    def __init__(self) -> None:
        ...
        # `_lock` so registration and the revision check are one atomic step.
    ...
    def bump(self) -> int:
        ...
        # and a bump must never hold `_lock` across another lock.
